...
def cropping(image_path: str, destination_dir: str = '.'):
    ...
    width_cm = 7 * CM_TO_PIXEL
   # width_cm = 26 * CM_TO_PIXEL
    height_cm = height_pixel * width_cm / width_pixel

    # resize image
    resized_image = origin_image.resize((int(width_cm), int(height_cm)))

    # get resized size
    _, new_height_pixel = resized_image.size

    # cm -> pixel
    cut_height_pixel = CUT_HEIGHT_CM * CM_TO_PIXEL
    ...
    for i in range(crop_ct):
        ...
        cropped_image.save(f'{dest_directory}/{i}.png')
